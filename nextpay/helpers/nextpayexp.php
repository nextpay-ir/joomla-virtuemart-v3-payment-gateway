...
class nextpayHelpernextpayExp extends nextpayHelpernextpay
{
    public $api_login_id = '';
    ...
    {
        parent::__construct($method, $nextpayPlugin);
        //Set the credentials
        ...
        // if ((!$this->ExpCredentialsValid() OR !$this->isAacceleratedOnboardingValid())) {
            // $text = vmText::sprintf('VMPAYMENT_nextpay_CREDENTIALS_NOT_SET', $this->_method->payment_name, $this->_method->virtuemart_paymentmethod_id);
            // vmError($text, $text);
        // }
        // if (empty ($this->_method->expected_maxamount)) {
            // $text = vmText::sprintf('VMPAYMENT_nextpay_PARAMETER_REQUIRED', vmText::_('VMPAYMENT_nextpay_EXPECTEDMAXAMOUNT'), $this->_method->payment_name, $this->_method->virtuemart_paymentmethod_id);
            // vmError($text, $text);
        // }
    }
    ...
    public function initPostVariables($nextpayMethod)
    {
        $post_variables = [];
        $post_variables['METHOD'] = $nextpayMethod;
        $post_variables['version'] = '104.0';
        // 104.0 required by nextpay
        //https://developer.nextpay.com/webapps/developer/docs/classic/release-notes/
    ...
    /**
     * https://developer.nextpay.com/webapps/developer/docs/classic/api/merchant/SetExpressCheckout_API_Operation_NVP/.
     *
    ...
    /*
     * languages supported according to this https://cms.nextpay.com/uk/cgi-bin/?cmd=_render-content&content_ID=developer/e_howto_api_ECCustomizing
     */
    public function getLocaleCode()
    {
        ...
        if (isset($explode[1])) {
            $country = $explode[1];
            if (in_array($country, $nextpayLanguages)) {
                return $country;
            }
        }
    ...
    public function getToken()
    {
        ...
        if ($this->_method->virtuemart_paymentmethod_id == 0) {
            $msg = 'Programming error,nextpay expresscheckout: virtuemart_paymentmethod_id is 0';
            vmError($msg, $msg);
        ...
        if ($this->_method->payment_type == '_xclick-subscriptions') {
            ...
            if ($this->total == 0) {
                $msg = 'Programming error,nextpay expresscheckout: total sent is 0';
                vmError($msg, $msg);
        ...
        // It is almost impossible to have the same amount as the one calultaed by nextpay
    ...
    public function getExpressCheckoutDetails()
    {
        ...
        if ($this->handleResponse()) {
            ...
            $this->customerData->setVar('nextpay_response', $this->response);
            $this->customerData->save();
            $this->storeAddresses();
    ...
    public function DoPayment()
    {
        ...
        $this->sendRequest($post_variables);
        // https://developer.nextpay.com/webapps/developer/docs/classic/express-checkout/ht_ec_fundingfailure10486/
        ...
        if (!$responseValid) {
            if ($this->response['L_ERRORCODE0'] == self::FRAUD_FAILURE_ERROR_CODE and $this->_method->payment_action == 'Sale' and $redirect <= 2) {
                $redirect++;
                // redirect buyer to nextpay
                $this->redirectTonextpay($post_variables['TOKEN']);
            }
    ...
    {
        //https://developer.nextpay.com/webapps/developer/docs/classic/direct-payment/ht_dp-recurringPaymentProfile-curl-etc/
        //https://developer.nextpay.com/webapps/developer/docs/classic/api/merchant/CreateRecurringPaymentsProfile_API_Operation_NVP/
    ...
    public function ManageRecurringPaymentsProfileStatus($payment)
    {
        ...
        $post_variables['PROFILEID'] = $nextpay_data->PROFILEID;
        $post_variables['ACTION'] = 'Cancel';
        $post_variables['TOKEN'] = $nextpay_data->TOKEN;
        $post_variables['PAYERID'] = $nextpay_data->payer_id;

        $this->sendRequest($post_variables);
        $this->handleResponse();

        return $this->GetRecurringPaymentsProfileDetails($nextpay_data->PROFILEID);
    }
    ...
    public function DoCapture($payment)
    {
        ...
        if (strcasecmp($nextpay_data->PAYMENTINFO_0_PAYMENTSTATUS, 'Pending') != 0 && strcasecmp($nextpay_data->PAYMENTINFO_0_PENDINGREASON, 'Authorization') != 0) {
            return false;
        }
        ...
        $reauth = $this->doReauthorize($nextpay_data->PAYMENTINFO_0_TRANSACTIONID, $nextpay_data);
        if ($reauth === false) {
            $post_variables['AUTHORIZATIONID'] = $nextpay_data->PAYMENTINFO_0_TRANSACTIONID;
        } else {
    ...
    public function doReauthorize($AuthorizationID, $nextpay_data)
    {
        // TODO
        return false;
        ...
        $post_variables['AMT'] = $this->total;
        $post_variables['CURRENCYCODE'] = $nextpay_data->PAYMENTINFO_0_CURRENCYCODE;
    ...
    {
        $nextpay_data = json_decode($payment->nextpay_fullresponse);
        if ($payment->nextpay_response_payment_status == 'Completed') {
            $post_variables = $this->initPostVariables('RefundTransaction');
            $post_variables['REFUNDTYPE'] = 'Full';
        } else {
            if ($payment->nextpay_response_payment_status == 'Pending' && $payment->nextpay_response_pending_reason == 'authorization') {
        ...
        $post_variables['AuthorizationID'] = $payment->nextpay_response_txn_id;
        $post_variables['TRANSACTIONID'] = $payment->nextpay_response_txn_id;
        $post_variables['TOKEN'] = $nextpay_data->TOKEN;
        $post_variables['PAYERID'] = $nextpay_data->payer_id;
     ...
     * https://developer.nextpay.com/docs/classic/express-checkout/ht_ec_fundingfailure10486/.
     *
     * @return bool
     */
    ...
    public function storeAddresses()
    {
        ...
        $addressST['shipto_address_type_name'] = 'nextpay Account';
    ...
    public function redirectTonextpay($token = '')
    {
        $useraction = '';
        ...
        if ($browser->isMobile()) {
            $url = $this->_getnextpayUrl().'?cmd=_express-checkout-mobile&token='.$token.$useraction;
        } else {
            $url = $this->_getnextpayUrl().'?cmd=_express-checkout&token='.$token.$useraction;
        }

        if ($this->_method->debug) {
            echo '<div style="background-color:red;color:white;padding:10px;">The method is in debug mode. <a href="'.$url.'">Click here to be redirected to nextpay</a></div>';
            jexit();
        } else {
            //header('location: ' . $url);
    ...
    public function validate($enqueueMessage = true)
    {
        ...
        if (!$this->customerData->getVar('token')) {
            $this->getToken();
            //Code stops here as the getToken method should redirect to nextpay
    ...
    /**
     * This page returns a 404 https://www.nextpay.com/mx/cgi-bin/?cmd=xpt/Merchant/merchant/ExpressCheckoutButtonCode-outside.
     *
     * code form here
     * https://www.nextpayobjects.com/IntegrationCenter/ic_express-buttons.html
     *
     * @return array
     */
    ...
    public function getExpressProduct()
    {
        ...
        $lang_iso = str_replace('-', '_', $lang->gettag());
        $nextpay_buttonurls = [
            'en_US' => 'https://www.nextpay.com/en_US/i/logo/nextpay_mark_60x38.gif',
            'en_GB' => 'https://www.nextpay.com/en_GB/i/bnr/horizontal_solution_PP.gif',
            'de_DE' => 'https://www.nextpay.com/de_DE/DE/i/logo/lockbox_150x47.gif',
            'es_ES' => 'https://www.nextpayobjects.com/WEBSCR-600-20100105-1/en_US/FR/i/bnr/bnr_horizontal_solution_PP_327wx80h.gif',
            'pl_PL' => 'https://www.nextpayobjects.com/WEBSCR-600-20100105-1/en_US/FR/i/bnr/bnr_horizontal_solution_PP_327wx80h.gif',
            'nl_NL' => 'https://www.nextpayobjects.com/WEBSCR-600-20100105-1/en_US/FR/i/bnr/bnr_horizontal_solution_PP_327wx80h.gif',
            'fr_FR' => 'https://www.nextpayobjects.com/WEBSCR-600-20100105-1/en_US/FR/i/bnr/bnr_horizontal_solution_PP_327wx80h.gif',
            'it_IT' => 'https://www.nextpayobjects.com/WEBSCR-600-20100105-1/it_IT/IT/i/bnr/bnr_horizontal_solution_PP_178wx80h.gif',
            'zn_CN' => 'https://www.nextpayobjects.com/WEBSCR-600-20100105-1/en_US/FR/i/bnr/bnr_horizontal_solution_PP_327wx80h.gif',
        ];
        $nextpay_infolink = [
            'en_US' => 'https://www.nextpay.com/us/cgi-bin/webscr?cmd=xpt/Marketing/popup/OLCWhatIsnextpay-outside',
            'en_GB' => 'https://www.nextpay.com/uk/cgi-bin/webscr?cmd=xpt/Marketing/popup/OLCWhatIsnextpay-outside',
            'de_DE' => 'https://www.nextpay.com/de/cgi-bin/webscr?cmd=xpt/Marketing/popup/OLCWhatIsnextpay-outside',
            'es_ES' => 'https://www.nextpay.com/es/cgi-bin/webscr?cmd=xpt/Marketing/popup/OLCWhatIsnextpay-outside',
            'pl_PL' => 'https://www.nextpay.com/pl/cgi-bin/webscr?cmd=xpt/Marketing/popup/OLCWhatIsnextpay-outside',
            'nl_NL' => 'https://www.nextpay.com/nl/cgi-bin/webscr?cmd=xpt/Marketing/popup/OLCWhatIsnextpay-outside',
            'fr_FR' => 'https://www.nextpay.com/fr/cgi-bin/webscr?cmd=xpt/Marketing/popup/OLCWhatIsnextpay-outside',
            'it_IT' => 'https://www.nextpay.com/it/cgi-bin/webscr?cmd=xpt/Marketing/popup/OLCWhatIsnextpay-outside',
            'zn_CN' => 'https://www.nextpay.com/cn/cgi-bin/webscr?cmd=xpt/Marketing/popup/OLCWhatIsnextpay-outside',
        ];
        if (!isset($nextpay_buttonurls[$lang_iso])) {
            $lang_iso = 'en_US';
        }
        $nextpayProduct['link'] = $nextpay_infolink[$lang_iso];
        $nextpayProduct['img'] = $nextpay_buttonurls[$lang_iso];

        return $nextpayProduct;
    }
