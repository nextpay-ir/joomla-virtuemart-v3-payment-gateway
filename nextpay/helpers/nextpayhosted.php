<?php
/**
 * nextpay  Hosted Pro payment plugin.
 *
...
class nextpayHelpernextpayHosted extends nextpayHelpernextpay
{
    // Pay Now button; since version 65.1
    const BM_BUTTON_TYPE = 'PAYMENT';
    //A secure button, not stored on nextpay, used only to initiate the Hosted Solution checkout flow;
    //default for Pay Now button. Since version 65.1
    const BM_BUTTON_CODE = 'TOKEN';
    const BM_BUTTON_VERSION = '104.0';
    const nextpay_USER_LG = 64;
    public $api_login_id = '';
    ...
    public function __construct($method, $nextpayPlugin)
    {
        ...
        // if ((empty ($this->_method->payflow_partner) OR empty($this->_method->sandbox_payflow_partner))) {
            ...
            // if ($this->_method->sandbox  ) {
                // $sandbox = 'SANDBOX_';
            // }
            // $text = vmText::sprintf('VMPAYMENT_nextpay_PARAMETER_REQUIRED', vmText::_('VMPAYMENT_nextpay_' . $sandbox . 'PAYFLOW_PARTNER'), $this->_method->payment_name, $this->_method->virtuemart_paymentmethod_id);
            // vmError($text);
        // }
    }
    ...
    // todo check the nextpay langauge: can it be sent. Atm sent in the country lanaguge
    // verfiez la langue, à cause accent
    public function initPostVariables($nextpayMethod)
    {
        $post_variables = [];
        $post_variables['METHOD'] = $nextpayMethod;
        $post_variables['VERSION'] = self::BM_BUTTON_VERSION; //https://developer.nextpay.com/webapps/developer/docs/classic/release-notes/
        ...
        $post_variables['BUTTONIMAGEURL'] = 'https://www.nextpay.com/en_US/i/btn/btn_paynow_SM.gif'; //we automatically redirect to nextpay
    ...
    {
        $post_variables['L_BUTTONVAR']['return'] = JURI::root().'index.php?option=com_virtuemart&view=pluginresponse&task=pluginresponsereceived&nextpayproduct=hosted&on='.$this->order['details']['BT']->order_number.'&pm='.$this->order['details']['BT']->virtuemart_paymentmethod_id.'&Itemid='.vRequest::getInt('Itemid').'&lang='.vRequest::getCmd('lang', '');
        ...
        $post_variables['L_BUTTONVAR']['cancel_return'] = JURI::root().'index.php?option=com_virtuemart&view=pluginresponse&task=pluginUserPaymentCancel&nextpayproduct=hosted&on='.$this->order['details']['BT']->order_number.'&pm='.$this->order['details']['BT']->virtuemart_paymentmethod_id.'&Itemid='.vRequest::getInt('Itemid').'&lang='.vRequest::getCmd('lang', '');
    }
    ...
    public function redirectTonextpay()
    {
        ...
        if ($this->_method->debug and $this->_method->template != 'templateD') {
            echo '<div style="background-color:red;color:white;padding:10px;">The method is in debug mode. <a href="'.$emailink.'">Click here to be redirected to nextpay</a></div>';
            echo '<div style="background-color:red;color:white;padding:10px;">The method is in debug mode. '.$websitecode.'Click here to be redirected to nextpay</a></div>';
            jexit();
        } else {
    ...
    public function DoCapture($payment)
    {
        ...
        if ($nextpay_data->payment_status != 'Pending' && $nextpay_data->pending_reason != 'Authorization') {
            return false;
        }
        $post_variables = $this->initPostVariables('DoCapture');

        //Do we need to reauthorize ?
        $reauth = $this->doReauthorize($payment->nextpay_response_txn_id, $nextpay_data);

        // the authorisation identification number of the payment you want to capture.
        if ($reauth === false) {
            $post_variables['AUTHORIZATIONID'] = $nextpay_data->txn_id;
        } else {
...
// (Optional) Your invoice number or other identification number that is displayed to the merchant and customer in his transaction history.
    ...
    /**
     * https://developer.nextpay.com/webapps/developer/docs/classic/nextpay-payments-standard/integration-guide/authcapture/.
     *
    ...
    public function doReauthorize($AuthorizationID, $nextpay_data)
    {
        ...
        $post_variables['PAYERID'] = $nextpay_data->payer_id; // Unique nextpay customer account identification number
        $post_variables['AUTHORIZATIONID'] = $AuthorizationID;
        $post_variables['PAYMENTACTION'] = 'DoReauthorization';
        $post_variables['AMT'] =  $nextpay_data->mc_gross; // ???
        $post_variables['CURRENCYCODE'] = $nextpay_data->mc_currency;
    ...
    {
        $nextpay_data = json_decode($payment->nextpay_fullresponse);
        if (strcasecmp($nextpay_data->payment_status, 'Completed') == 0) {
            $post_variables = $this->initPostVariables('RefundTransaction');
            $post_variables['REFUNDTYPE'] = 'Full';
            $post_variables['TRANSACTIONID'] = $nextpay_data->txn_id;
        } elseif (strcasecmp($nextpay_data->payment_status, 'Pending') == 0 && strcasecmp($nextpay_data->pending_reason, 'authorization') == 0) {
