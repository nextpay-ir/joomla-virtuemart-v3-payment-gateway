...
class nextpayHelpernextpayStd extends nextpayHelpernextpay
{
    ...
    public function __construct($method, $nextpayPlugin)
    {
        ...
        // if (empty($this->merchant_email)) {
            ...
            // if ($this->_method->sandbox) {
                // $sandbox = 'SANDBOX_';
            // }
            // $text = vmText::sprintf('VMPAYMENT_nextpay_PARAMETER_REQUIRED', vmText::_('VMPAYMENT_nextpay_' . $sandbox . 'MERCHANT'), $this->_method->payment_name, $this->_method->virtuemart_paymentmethod_id);
            // vmError($text, $text);
            // return FALSE;
        // }
    }
    ...
    public function preparePost()
    {
        ...
        switch ($this->_method->payment_type) {
            ...
            case '_xclick-subscriptions':
                ...
                $post_variables['sra'] = 1; //Reattempt on failure. If a recurring payment fails, nextpay attempts to collect the payment two more times before canceling the subscription.
        ...
        $url = $this->_getnextpayUrl();
        if (vmconfig::get('css')) {
            $msg = vmText::_('VMPAYMENT_nextpay_REDIRECT_MESSAGE', true);
        } else {
            $msg = '';
        }
        ...
        if ($this->_method->debug) {
            $html .= '<form action="'.$url.'" method="post" name="vm_nextpay_form" target="nextpay">';
        } else {
            $html .= '<form action="'.$url.'" method="post" name="vm_nextpay_form" id="vmPaymentForm" accept-charset="UTF-8">';
        }
        ...
        if ($this->_method->debug) {
            $html .= '<div style="background-color:red;color:white;padding:10px;">
						<input type="submit"  value="The method is in debug mode. Click here to be redirected to nextpay" />
						</div>';
            $this->debugLog($post_variables, 'nextpay request:', 'debug');
        } else {
            $html .= '<input type="submit"  value="'.vmText::_('VMPAYMENT_nextpay_REDIRECT_MESSAGE').'" />';
        }
        $html .= '</form>';
    ...
    public function initPostVariables($payment_type)
    {
        ...
        if ($payment_type == '_xclick') {
            $post_variables['address_override'] = $this->_method->address_override; // 0 ??   nextpay does not allow your country of residence to ship to the country you wish to
        }
...
// TODO Check that paramterer
    ...
    public function onShowOrderBEPaymentByFields($payment)
    {
        ...
        foreach ($showOrderBEFields as $key => $showOrderBEField) {
            ...
            if ($payment->$field) {
                $html .= $this->nextpayPlugin->getHtmlRowBE($prefix.$key, $payment->$field);
            }
        }
