...
<br />
<style>
	.nextpayordersummary td {padding:10px;}
</style>
<table cellpadding="2" class="nextpay_ordersummary">
	<?php 
    echo $this->getHtmlRow('VMPAYMENT_nextpay_API_PAYMENT_NAME', $viewData['payment_name']);
    echo $this->getHtmlRow('COM_VIRTUEMART_ORDER_NUMBER', $response['invoice']);
    echo $this->getHtmlRow('VMPAYMENT_nextpay_API_AMOUNT', $response['PAYMENTINFO_0_AMT'].' '.$response['PAYMENTINFO_0_CURRENCYCODE']);

    if ($viewData['success']) {
        echo $this->getHtmlRow('VMPAYMENT_nextpay_API_TRANSACTION_ID', $response['PAYMENTINFO_0_TRANSACTIONID']);
    } else {
        for ($i = 0; isset($response['L_ERRORCODE'.$i]); $i++) {
            echo $this->getHtmlRow('VMPAYMENT_nextpay_API_ERROR_CODE', $response['L_ERRORCODE'.$i]);
