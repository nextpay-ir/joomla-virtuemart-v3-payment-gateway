...
<style>
	.nextpay_ordersummary td {padding:10px;}
</style>
<div class="nextpay_ordersummary">
<table cellpadding="2" class="nextpay_ordersummary">
	<?php 
    echo $this->getHtmlRow('VMPAYMENT_nextpay_API_PAYMENT_NAME', $payment_name);
    echo $this->getHtmlRow('COM_VIRTUEMART_ORDER_NUMBER', $viewData['order']['details']['BT']->order_number);
    echo $this->getHtmlRow('VMPAYMENT_nextpay_API_AMOUNT', $responseData['AMT'].' '.$responseData['CURRENCYCODE']);

    if ($viewData['success']) {
        echo $this->getHtmlRow('VMPAYMENT_nextpay_API_TRANSACTION_ID', $responseData['TRANSACTIONID']);
    } else {
        for ($i = 0; isset($responseData['L_ERRORCODE'.$i]); $i++) {
            echo $this->getHtmlRow('VMPAYMENT_nextpay_API_ERROR_CODE', $responseData['L_ERRORCODE'.$i]);
            $message = isset($responseData['L_LONGMESSAGE'.$i]) ? $responseData['L_LONGMESSAGE'.$i] : $responseData['L_SHORTMESSAGE'.$i];
            echo $this->getHtmlRow('VMPAYMENT_nextpay_API_ERROR_DESC', $message);
        }
    }
    ?>
</table>
</div>
<div class="nextpay_orderlink">
